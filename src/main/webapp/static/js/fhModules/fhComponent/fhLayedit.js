...
layui.define(['layer', 'form'], function(exports){
  ...
  ,hint = layui.hint()
  ,device = layui.device()
  ,MOD_NAME = 'fhLayedit', THIS = 'layui-this', SHOW = 'layui-show', ABLED = 'layui-disabled'
  ...
  ,toolActive = function(iframeWin, editor, set){
    ...
    ,toolEvent = {
      ...
      ,image: function(range){
      	if(set.uploadImage && !set.uploadImage.multiple){
        var that = this;
    ...
    // 判断是多图还是单图
    if(set.uploadImage && set.uploadImage.multiple){
    	toolEvent.bindUpload();
    }else{
    	$("#kdupliadinput").attr("type","file");
    }
  ...
  ,tools = {
    ...
    ,image: '<i id="kduploadBtn" class="layui-icon layedit-tool-image" title="图片" layedit-event="image">&#xe64a;<input id="kdupliadinput" name="file"></i>'
